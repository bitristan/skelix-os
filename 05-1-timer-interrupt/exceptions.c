...
/*
 堆栈中元素顺序，从高地址到低地址: (当前call指令压入的下一条指令地址不算)
    eflags
    cs
    eip
    err
    ...
    kp_level
 */
void info(enum KP_LEVEL kl, unsigned int ret_ip,
	 unsigned int old_ebp, unsigned int ss, unsigned int gs, unsigned int fs, 
